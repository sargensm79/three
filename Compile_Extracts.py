...
extract_files = [fn for fn in os.listdir("/home/ubuntu/") if fn.startswith("AUTOGRAPH_201")]
...
with open(base_output, "a") as base_reader:
    for ef in to_append:
        for chunk in pd.read_csv(ef, sep=";", quotechar='"', quoting=csv.QUOTE_ALL, chunksize=10000):
            chunk.to_csv(base_reader, header=False, sep=";", quotechar='"', quoting=csv.QUOTE_ALL)
